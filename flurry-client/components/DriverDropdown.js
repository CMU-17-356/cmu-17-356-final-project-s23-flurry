...
const DriverDropdown = ({ onSelectDriver }) => {
  ...
  const renderDriverItem = ({ item }) => (
    <TouchableOpacity style={styles.driverItem} onPress={() => handleDriverSelect(item)}>
    ...
    <View style={styles.container}>
      ...
      {selectedDriver ? (
        <Text style={styles.driverText}>
          Number of slip incidents for {selectedDriver}: {dummyDrivers.find((driver) => driver.name === selectedDriver).slipIncidents}
        </Text>
      ) : (
        <>
          ...
          <View style={styles.genericDataContainer}>
            <Text style={styles.genericDataText}>
              Driver with the most number of slip incidents: Max
            </Text>
            <Text style={styles.genericDataText}>
              Driver with the least number of slip incidents: Peter
            </Text>
            <Text style={styles.genericDataText}>Total number of slips in the given date range: 25</Text>
          </View>
        </>
      )}
    </View>
